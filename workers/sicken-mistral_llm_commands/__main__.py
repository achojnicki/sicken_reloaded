...
class Mistral_LLM_Commands:
	project_name="sicken-mistral_llm_commands"
	...
	def __init__(self):
		...
		self._log.info('Initialising module sicken-mistral_llm_commands worker')
		...
		self._mistral=OpenAI(
			api_key=self._config.mistral.api_key, 
		...
		self._log.success('Worker sicken-mistral_llm_commands initialised successfully')
		...
		try:
			...
			for message in previous_messages:
				...
				if message['message_author'] == 'Sicken.ai':
					...
					m={
						"role": "tool" if self._config.mistral.use_tools_calls else "function",
						"name": message['func_name'],
						"content": dumps(message['message']),
					}

						
					if self._config.mistral.use_tools_calls:
	...
	def _get_model_response(self, prompt):
		try:
			...
			completion=self._mistral.chat.completions.create(**args)
...
if __name__=="__main__":
	Mistral_LLM_Commands=Mistral_LLM_Commands()
	Mistral_LLM_Commands.start()
