...
run "apt-get install curl gnupg apt-transport-https python3 python3-pip nginx curl dpkg-dev build-essential libjpeg-dev libtiff-dev libsdl1.2-dev libgstreamer-plugins-base1.0-dev libnotify-dev freeglut3-dev libsm-dev libgtk-3-dev libwebkit2gtk-4.1-dev libxtst-dev libsdl2-dev ffmpeg -y"

print "2nd stage installation of dependencies(may take a while)"
run "python3.13 -m pip install --break-system-packages numpy openai flask flask-socketio python-socketio psutil tabulate colored pymongo pyyaml pika uwsgi websockets wxpython mistune eventlet firecrawl"
...
run "rabbitmqctl add_user sicken-llamacpp_llm password"
run "rabbitmqctl add_user sicken-deepseek_llm password"
run "rabbitmqctl add_user sicken-mistral_llm password"
...
rabbitmqctl set_permissions -p / sicken-llamacpp_llm ".*" ".*" ".*"
rabbitmqctl set_permissions -p / sicken-deepseek_llm ".*" ".*" ".*"
rabbitmqctl set_permissions -p / sicken-mistral_llm ".*" ".*" ".*"
...
rabbitmqctl set_topic_permissions sicken-llamacpp_llm "" ".*" ".*"
rabbitmqctl set_topic_permissions sicken-deepseek_llm "" ".*" ".*"
rabbitmqctl set_topic_permissions sicken-mistral_llm "" ".*" ".*"
...
run 'create_queue.py sicken-search_requests'
run 'create_queue.py sicken-search_feedback'
